...
\begin{enumerate}
    ...
    Let's look at the case where $y=-1$, here $\Phi(x) = -1$. By using our value of $W$ from before we can now write: $w_b(-1) + w_0 = 0$ for a point at the decision boundary.
    ...
    We can simplify the optimisation to only work over the $w_0$ from part 4, $x_{2i}$ represents the $i^{th}$ element of x in the second row (i.e. only those transformed by the basis function). 
    \begin{align*}
        &\min_{\mathbf{w_0}} \mathbf{w_0} \quad \text{s.t.} \quad & y_i(w_0 \bphi(x_{2i}) + w_0) \geq 1,~\forall i \in \{1,\ldots, n\} \\
        &= \min_{\mathbf{w_0}} \mathbf{w_0} \quad \text{s.t.} \quad & y_i w_0 ( \bphi(x_{2i}) + 1) \geq 1,~\forall i \in \{1,\ldots, n\} \\
    \end{align*}
    ...
    \begin{align*}
        ...
        h(\bphi(x);\boldw,w_0) &= 1\bphi(x_2) + 1 \\
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \includegraphics[width=10cm]{hw4/img/p3_1.png}
        \centering
    \end{figure}
\end{enumerate}
