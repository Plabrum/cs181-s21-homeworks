...
\begin{enumerate}
    ...
    Now we can substitute the kernel into the L2 function:
    $$
        \mcL(W) =  \sum^{n}_{a = 1}{\left[
            \left(y_a - \left( \frac  {\sum^n_{b=1} exp(-(x_a -x_b)^T W(x_a - x_b)) y_n}
                            {\sum^n_{b=1} exp(-(x_a -x_b)^T W(x_a - x_b))}\right)
            \right)^2
        \right]}
    $$
    ...
    $$
        \mcL(W) = \sum^{n}_{a = 1}{\left[
            \left(y_a - \left( \frac  {\sum^n_{b=1} exp(-(z)^T W(z)) y_n}
                            {\sum^n_{b=1} exp(-(z)^T W(z)})\right)
            \right)^2
        \right]}
    $$
    ...
    $$
        \mcL(W) = \sum^{n}_{a = 1}{\left[
            \left(y_a - \left( \frac  {\sum^n_{b=1} exp(-(w_{11}z_{i}^{2} + 2w_{12}z_{i}z_{j} + w_{22}z_j^2)) y_n}
                            {\sum^n_{b=1} exp(-(w_{11}z_{i}^{2} + 2w_{12}z_{i}z_{j} + w_{22}z_j^2))}\right)
            \right)^2
        \right]}
    $$
    
    \item Derive the gradients of the Loss function.
...
\begin{enumerate}
    ...
    \item When k=1 there will be a dissagreement between kernel based and kNN
    
    \item Alpha increases the distance of far away points (i.e. for a given distance away the kernel will duiscrimintate it further) When using kNN we can only select one of the nearest k points, so we have no need to furhter reduce the impact of far points using alpha.
...
No collaborators or external resources
