...
\begin{enumerate}
    ...
    \begin{align*}
        ...
        % \sum_n^N \log p(\{x_n\}^N_{n=1}; \btheta, \{p_k\}^K_{k = 1}) &= \sum_n^N \log p(x_n; \btheta, \{p_k\}^K_{k = 1}) \\
        % \log p(\{x_n\}^N_{n=1}; \btheta, \{p_k\}^K_{k = 1}) = 
    \end{align*}
    ...
    \end{align*}
    % \begin{enumerate}
    %     \item A $q_n$ p(z_n = c_1)
    %     we are given that z_n and x_n in the problem (what does geometric mean)
    %     1. use bayes rule
    %     2. plug in pdfs
    %     3. normalize (divide by sum of components in q_n)
    %     \item Concretre algorithm means give pseudocode
    % \end{enumerate}
    % \item 
    % \begin{enumerate}
    %     \item use our q_n to derive our (similar sections 8 notes)
    % \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{figure}[H]
            \includegraphics[height=18cm]{hw5/plots/p2_1a.png}
        \centering
    \end{figure}
    \end{enumerate}
\end{enumerate}
